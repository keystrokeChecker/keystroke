...
class AppSettings {
  ...
  const AppSettings({this.backendUrl = '', this.method = 'rule'});
...
class LocalStore {
  ...
  Future<AppSettings> loadSettings() async {
    ...
    if (encoded != null) {
      try {
        ...
        if (decoded is Map) {
          ...
          if (backendUrl is String && method is String) {
            return AppSettings(backendUrl: backendUrl, method: method);
          }
        }
      } on Object {
    ...
    return AppSettings(
      ...
      method: _readString(methodKey) ?? 'rule',
    );
  }
