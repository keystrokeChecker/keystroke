...
void main() {
  ...
  group('testConnection', () {
    ...
    test('fetches model info from model-info endpoint', () async {
      final client = BackendClient(
        baseUrl: 'http://localhost:8000',
        clientFactory: () => _RecordingClient((request) async {
          expect(request.method, 'GET');
          expect(request.url, Uri.parse('http://localhost:8000/model-info'));
      ...
      expect(info.modelName, 'ExtraTrees');
      expect(info.featureSet, 'D');
      expect(info.accuracy, 0.909);
      expect(info.macroF1, 0.914);
      expect(info.top3Accuracy, 0.971);
    });
  });
  ...
  group('analyze', () {
    ...
    test('uploads file and method and returns a typed response', () async {
      final client = BackendClient(
        ...
        clientFactory: () => _RecordingClient((request) async {
          ...
          return _jsonResponse(<String, dynamic>{
            'counts': <int>[5, 4, 6],
            'formatted': '5, 4, 6',
            'reconstructed_sequence': 'HELLO',
            'detected_events_count': 5,
            'events': [
              {
                'event_index': 0,
                'timestamp_s': 0.25,
      ...
      expect(result.reconstructedSequence, 'HELLO');
      expect(result.detectedEventsCount, 5);
      expect(result.events.length, 1);
      expect(result.events.first.prediction, 'H');
