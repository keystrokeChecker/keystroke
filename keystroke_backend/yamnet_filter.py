...
def pool_word_features(
    per_onset_features: np.ndarray,
    word_onset_times: np.ndarray | list[float],
) -> np.ndarray:
    """
    Pool the per-onset features of one word into a single feature vector.

    Mean-pooling alone is invariant to how many onsets were pooled, so a
    3-keystroke word and a 6-keystroke word collapse to vectors of the same
    shape and scale — the count is destroyed. The four appended scalars
    restore it, letting the regressor learn a correction to the raw onset
    count rather than guessing the count from timbre alone.

    Parameters
    ----------
    per_onset_features : (n_onsets, 1028)
        Rows of `extract_features` output belonging to this word.
    word_onset_times : (n_onsets,)
        Onset timestamps for the same rows, in seconds.

    Returns
    -------
    (1032,) float32 — 1028-D mean-pool + [n_onsets, duration, mean_gap, std_gap]
    """
    if len(per_onset_features) == 0:
        raise ValueError("pool_word_features requires at least one onset")

    times = np.sort(np.asarray(word_onset_times, dtype=float))
    n_onsets = float(len(times))
    ...
    return np.concatenate([
        per_onset_features.mean(axis=0),
        [n_onsets, duration, mean_gap, std_gap],
    ]).astype(np.float32)
